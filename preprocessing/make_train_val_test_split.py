...
def sample_even_distribution(
    df_in: pd.DataFrame, metadata_df: pd.DataFrame, n_samples=3
):
...
if __name__ == "__main__":
    ...
    print(
        df_both_audio_and_transcripts.groupby(["Diagnosis", "Gender"])["ID"].describe()
    )

    ## Remove TDs from dataset before sampling
    df_both_audio_and_transcripts_no_td = df_both_audio_and_transcripts[
        df_both_audio_and_transcripts["Diagnosis"] != "TD"
    ]
    # remove the F/M gender to be able to stratify sampling
    df_both_audio_and_transcripts_no_td = df_both_audio_and_transcripts_no_td[
        df_both_audio_and_transcripts_no_td["Gender"] != "F/M"
    ]
    ...
    df_both_audio_and_transcripts_no_td_no_test = (
        df_both_audio_and_transcripts_no_td.drop(
            df_test[df_test["Diagnosis"] != "TD"].index
        )
    )

    # sample for validation set
    df_val = sample_even_distribution(
        df_both_audio_and_transcripts_no_td_no_test, df_no_test
    )
